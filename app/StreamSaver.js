		...
		setupChannel = () => new Promise((resolve, reject) => {
			channel.port1.onmessage = evt => {
				if(evt.data.download) {
					...
					link.setAttribute("download", "eeg.txt");
					link.target = "_blank"; // Opens the download in a new tab to prevent firing the unload/beforeunload events on the main page
